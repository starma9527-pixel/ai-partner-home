...
exports.handler = async (event, context) => {
  ...
  if (type === 'customer_analysis') {
    ...
    systemPrompt =
      ...
      '  "revenueYear": "营收数据对应年份，如2024或2025",\n' +
      '  "website": "公司官网URL",\n' +
    ...
    userPrompt = '请执行全部10轮搜索流程，联网搜索以下公司及其集团的完整情报，严格按JSON格式输出：\n' +
      ...
      (input?.productName ? '\n产品/APP名称：' + input.productName : '') +
      (input?.website ? '\n公司官网：' + input.website : '') +
      '\n\n【重要提示】必须先搜索该公司股权关系，判断是否有母公司/集团。有则继续搜集团规模和云AI使用情况。特别关注：竞对云/AI产品使用情况（有助于判断替换阿里云的机会）、AI战略信号（判断AI采购意愿）、近期动态（判断当前是否处于关键决策节点）。\n\n请直接输出JSON对象，不要包含任何其他文字。';
